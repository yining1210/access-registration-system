<template>
  <div id="index">
    <mt-header title="人员出行登记系统" fixed>
    ...
    <div class="content-div">
      <ul>
        <li>
          <span>最近更新日期： {{data.createTime}}</span>
        </li>
      </ul>
    </div>
    ...
    <ul>
      ...
      <li>
        <span>是否本地户籍: {{data.isLocal?"是":"否"}}</span>
        <span>是否来自湖北: {{data.isFromhubei?"是":"否"}}</span>
      </li>
      <li>
      ...
      <li>
        ...
        <span>春运出行方式: {{data.travelMode==1?"火车":(data.travel_mode==2?"自驾":(data.travel_mode==3?"汽车":(data.travel_mode==4?"飞机":"无")))}}</span>
      </li>
      <li>
    ...
    <div v-for="(item,i) in visitinfo" :key="i">
      ...
      <ul>
        ...
        <li>
          ...
          <span>到访时间: {{item.createTime}}</span>
        </li>
      </ul>
    </div>
  </div>
...
<script>
import { peopleInfo } from "@/api/peopleInfo";
export default {
  data() {
    return {
      data: {},
