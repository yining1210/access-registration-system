...
get请求（获取人员列表信息）
/api/personinfo/?userid="xxxx" # 请注意一定要加userid字段
[                  # 注意这是[]
    {
        "id": 12,  # !!!这个有用的!!!主要用于下面的获取详细信息
        "visitinfo": [
            {
                "id": 1,
                "contactname": "林房东",
...
get请求（获取人员详细信息）
/api/personinfo/12/?userid="xxxx"  # 其中的/12/是列表获取数据中的"id":12
{             # 注意这是{}
    "id": 12,
    "visitinfo": [
        {
...
```
/api/personinfo/?search=程&userid="xxxx"    # 请注意一定要加userid字段
或/api/personinfo/?search=135805&userid="xxxx"  # 请注意一定要加userid字段
get请求

比如：/api/personinfo/?search=58367&userid="xxxx" # 请注意一定要加userid字段
返回响应：
200（即搜索成功）
{
    "count": 1,
